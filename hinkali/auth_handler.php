...
try {
    ...
    if ($action === 'login') {
        ...
        // Исправлено: используем fetch() вместо rowCount() для PostgreSQL
        if ($stmt->fetch()) {
            sendAuthResponse(false, 'Пользователь с таким email уже существует');
        }
        ...
        try {
            ...
            if ($newUser) {
                ...
                sendAuthResponse(false, 'Ошибка при создании пользователя');
            }
